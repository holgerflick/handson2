...
uses
  ...
  Modules.Resources, AdvGlowButton, AdvToolBar, VCL.TMSFNCGoogleMaps;
...
type
  TFrmMain = class(TForm)
    ...
    procedure btnAddClustersClick(Sender: TObject);
...
procedure TFrmMain.btnAddClustersClick(Sender: TObject);
begin
   ShowAllStates;
end;
...
procedure TFrmMain.ShowAllStates;
var
  LState : String;
  LDs : TDataset;
...
begin
  try
    Map.BeginUpdate;
    Map.Clear;
    LDs := DBController.QuStates;

    LDs.First;
    while not LDs.Eof do
    begin
      LState := LDs.FieldByName('state').AsString;
...
begin
  ...
  try
    ...
    begin
      LMarker := Map.AddMarker(
       CreateCoordinate( LQuery['Lat'], LQuery['Lon'] ), LQuery['county']
      ) as TTMSFNCGoogleMapsMarker;
