...
begin
  ...
  begin
    ...
    if LRow > 0 then
    begin
      if MessageDlg( 'Remove key?', mtConfirmation,
...
begin
  ...
  begin
    ...
    LScale := TTMSFNCUtils.GetDPIScale;

    LGlyph.TransparentMode := tmAuto;
    LGlyph.PixelFormat := pf32bit;
    LGlyph.Width := LGrid.RowHeights[ARow] - Trunc( 6 * LScale );
    LGlyph.Height := LGrid.RowHeights[ARow] - Trunc( 6 * LScale );
    ...
    LGrid.BtnEdit.ButtonWidth := Trunc( 50 * LScale );
    ShowMessage( LScale.ToString );
